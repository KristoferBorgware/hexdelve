/*
 * The bat's skeleton, as plain data.
 *
 * The humanoid rig is a spine with four limbs hanging off it, and every bone
 * in it points down -Y. Nothing about this creature fits that: its arms ARE
 * its wings, they run out along +-X rather than down, and the interesting
 * joints are the three folds along each wing that let two and a half metres of
 * span collapse into something that fits on one hexagon.
 *
 * So it gets its own rig, in the same shape of data — a name, a parent, an
 * offset — because that is all the engine's animation ever needed. Turn the
 * skeleton view on and this one draws itself from the list below, with no new
 * code anywhere.
 *
 ...
 *
 * which is why batpose.ts writes one wing and mirrors it.
 */
...
export const HOVER_Y = 0.72;
/** And where it settles when it comes down on its feet. */
export const PERCH_Y = 0.46;

export const BAT_SKELETON: Skeleton = [
...
export const BAT_BONES = boneNames(BAT_SKELETON);
export const BAT_BONE_INDEX = boneIndex(BAT_SKELETON);

export const BAT_TIPS: readonly BoneTip[] = [
 ...
 */
export const WING: Record<'L' | 'R', readonly string[]> = {
	L: ['armL', 'foreL', 'handL', 'digitL'],
	R: ['armR', 'foreR', 'handR', 'digitR'],
};

/** Tip of the outer finger in rest space, so the span is measured not guessed. */
export const SPAN = 2 * (0.11 + 0.34 + 0.4 + 0.26 + 0.24);

/** Where the jaws end up, in the jaw bone's own space. */
export const JAW_TIP: readonly [number, number, number] = [0, -0.02, 0.16];
