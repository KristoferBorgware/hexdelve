/*
 * The particle bench's canvas.
 *
 * Same shape as the three viewports next door, and for the same reason: a
 * canvas hands out one kind of context and then that is what it is forever, so
...
import type { BackendPreference, ParticleEffect } from '@hexdelve/engine';

import { ParticleBench } from './ParticleBench.js';

export interface ParticleViewportProps {
	backend: BackendPreference;
	running: boolean;
	/** What the bench is created on. Later changes go through `setEffect`. */
	effect: ParticleEffect;
	onBenchReady(bench: ParticleBench | null): void;
}
...
export function ParticleViewport({
	...
	const benchRef = useRef<ParticleBench | null>(null);
