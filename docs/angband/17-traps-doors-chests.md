...
The `DOWN` and `PIT` lines are **outside** the save branch. Feather Fall
...
These three share the trap list but lack `TRF_TRAP`, so nothing in
17.2–17.5 applies to them: they are not generated, not detected, not
disarmed, and they never fire on the player. How monsters treat them is in
*Chapter 12* 12.5.3. What that leaves for this chapter:

* **Glyph of warding** (`EF_GLYPH:WARDING`). A failed break roll does not
  end the monster's movement loop, so a monster gets one roll per
  *movement attempt*, not one per turn — a glyph delays a strong monster
  rather than stopping it.
* **Decoy** (`EF_GLYPH:DECOY`) also dies untouched if the player moves
  more than `max_sight` grids from it (`player_leaving()`).
* **Webs** are created only by monsters — `effect_handler_WEB()` returns
  false outright for a player caster — in a radius growing with the
  caster's spell power (1, +1 above 40, +1 above 80), on floor grids
  holding no other trap (`square_iswebbable()`). The **player** clears a
  web by walking into it, which spends the move.
...
`DAM_RED` is applied to the hit-point loss afterwards; the pack is damaged
from the raw figure instead (*Elements* 10.5).
