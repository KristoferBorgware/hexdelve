...
**Traps on the destination** are handled by `hit_trap()` in `trap.c` (*Traps* 17.4). Invisible traps are revealed by `square_reveal_trap()` when a seen grid is updated, provided `SKILL_SEARCH >= trap power`, so a low-search character can walk onto a trap they never saw.
...
Doors are opened (`o`), closed (`c`), locked (`D` on an unlocked closed door: `do_cmd_lock_door()`), or handled automatically by `+` (alter) and by walking into them. Walking into a closed door opens it rather than bumping, which is what makes a door cost a turn rather than a turn and a keypress.

The commands, the lock-picking chance and the lock powers are in *Traps and Doors* 17.6; where the generator puts doors and how secret ones are found is *Dungeon Generation* 16.6. Jammed doors (`DOOR_JAMMED`) exist as a flag but 4.2 places no spikes; monsters bash instead (*Chapter 12* 12.5.4).

## 6.6 Tunnelling

`do_cmd_tunnel()` (`T`), or alter on a diggable grid. The per-turn test is `chance > randint0(1600)`, so a chance of 1600 or more is certain; the chance per terrain class is in *Traps and Doors* 17.7 and the skill behind it in *Player Stats* 4.11.

What concerns movement is that a success calls `twall()`: the grid becomes floor, so a wall the player dug through is a permanent new route, and both the view and the noise and scent maps (6.10) are recomputed around it. What the dug grid yields is in 17.7.

## 6.7 Stairs and level changes

`<` and `>` require the matching staircase under the player; with `autoexplore_commands` they navigate to the nearest one instead of complaining. Each costs a turn and sets `create_up_stair` or `create_down_stair` so that, under `birth_connect_stairs`, `new_player_spot()` puts the connecting staircase under the player on arrival.

Where the stairs lead — `dungeon_get_next_level()`, `stair_skip`, the quest-level stop, and the other ways to change level — is *The World Loop* 19.2. The level itself is not built by the command: it is requested, and the main loop generates it when it is safe to (*Architecture* 1.6).
